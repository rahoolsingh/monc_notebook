...
import { Upload, File, Link, AlertCircle, CheckCircle, X, Trash2, FileText, Table, Play, Globe } from 'lucide-react';
  ...
  return (
    <div className="document-upload">
      <div className="upload-section">
        ...
        <div
          className={`upload-zone ${uploadState.isDragging ? 'dragging' : ''} ${isUploading ? 'uploading' : ''}`}
              ...
              <div key={doc.id} className="document-card">
                <div className="document-header">
                  ...
                  <div className="document-info">
                    <h4 className="document-name" title={doc.name}>{doc.name}</h4>
                    <p className="document-meta">
                      <span className={`document-type-badge ${doc.type}`}>
                        {doc.type.toUpperCase()}
                      </span>
                      {doc.size > 0 && ` • ${formatFileSize(doc.size)}`}
                    </p>
                    {doc.sourceUrl && (
                      <p className="document-source" title={doc.sourceUrl}>
                        {doc.sourceUrl.length > 50 ? doc.sourceUrl.substring(0, 50) + '...' : doc.sourceUrl}
                      </p>
                    )}
                  </div>
                ...
                {doc.uploadProgress !== undefined && doc.uploadProgress < 100 && (
                  <div className="progress-bar">
                    ...
                    />
                    <span className="progress-text">{doc.uploadProgress}%</span>
                  </div>
                )}
